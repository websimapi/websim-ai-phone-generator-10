...
function drawAppIcon(x, y, size, color, type) {
    ...
    switch (type) {
        case 'friends':
            ctx.beginPath();
            ctx.arc(-iSize * 0.25, -iSize * 0.1, iSize * 0.25, 0, Math.PI * 2);
            ctx.fill();
            ctx.beginPath();
            ctx.arc(iSize * 0.2, 0, iSize * 0.35, 0, Math.PI * 2);
            ctx.fill();
            break;
        case 'phone':
...
export function drawHomeScreen() {
    ...
    const colors = ['#4CAF50', '#2196F3', '#FF9800', '#E91E63', '#9C27B0', '#00BCD4', '#FF5722', '#607D8B', '#3F51B5'];
    const icons = ['phone', 'messages', 'music', 'browser', 'camera', 'settings', 'mail', 'clock', 'friends'];
